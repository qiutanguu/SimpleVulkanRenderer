...
// 处理shadowmap投影采样
float shadow_texture_proj(vec4 sample_clip_pos, vec2 offset, float NoL)
{
	float shadow = 1.0f;
	...
	{
		float dist = texture(directional_light_shadowdepth, shadow_coord.st + offset).r;
...
// 阴影 pcf 滤波
float shadow_pcf(vec4 sc,float NoL,float pixel_scale)
{
	...
	for (int x = 0; x < 128; x++)
	{
		shadow_factor += shadow_texture_proj(sc, vec2(dx, dy) * poisson128[x] * pixel_scale, NoL);
		count++;
	}
...
// pcss 查找遮挡物
float pcss_search_blocker(vec4 sc,float NoL,float pixel_scale) 
{
	ivec2 tex_dim = textureSize(directional_light_shadowdepth, 0).xy;
	float dx = 1.0 / float(tex_dim.x);
	float dy = 1.0 / float(tex_dim.y);
	...
	vec4 shadow_coord = sc / sc.w;
	shadow_coord.st = shadow_coord.st * 0.5f + 0.5f;

	for (int x = 0; x < 64; x++)
	{
		int index = int( 64.0 * random(shadow_coord.xyy, x) ) % 64;

		vec2 sample_uv = (vec2(pixel_scale) * poisson64[index]) * texel_size + shadow_coord.st;
		float dist = texture(directional_light_shadowdepth, sample_uv).r;
...
// pcss软阴影
float shadow_pcss(vec4 sc,float NoL)
{
	const float light_width = 25.0f;
	const float pixel_scale = 20.0f;

	vec4 shadow_coord = sc / sc.w;
	shadow_coord.st = shadow_coord.st * 0.5f + 0.5f;

	float avg_blocker_depth = pcss_search_blocker(sc, NoL, pixel_scale);
	float penumbra_size = max(shadow_coord.z - avg_blocker_depth,0.1) / avg_blocker_depth*light_width;

	return shadow_pcf(sc, NoL, penumbra_size);

}

vec3 compute_shadow(vec3 fragworldpos,float NoL)
{
	vec4 shadow_clip_pos = ub_directional_light_vp.proj * ub_directional_light_vp.view * vec4(fragworldpos, 1.0);
	float shadow_factor =  shadow_pcss(shadow_clip_pos,NoL);
...
void main() 
{
	...
	fragcolor = fragcolor * compute_shadow(world_pos,NoL) + vec3(0.05f) * basecolor.xyz;
